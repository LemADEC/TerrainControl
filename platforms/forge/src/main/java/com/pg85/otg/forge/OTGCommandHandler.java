...
final class OTGCommandHandler implements ICommand
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] argString)
    {
        ...
        if (!mcWorld.isRemote) // Server side
        {
            ...
            {
                sender.sendMessage(new TextComponentString("-- OTG --"));
                sender.sendMessage(new TextComponentString("Commands:"));
                sender.sendMessage(new TextComponentString("/otg worldinfo - Show author and description information for this world."));
                sender.sendMessage(new TextComponentString("/otg biome - Show biome information for any biome at the player's coordinates."));
            ...
            } else if (argString[0].equalsIgnoreCase("biome"))
            {
                ...
                final int savedId = biome instanceof OTGBiome ? ((OTGBiome) biome).id.getSavedId() : generationId;
            ...
            } else if (argString[0].equalsIgnoreCase("print"))
            {
                ...
                if (argString.length > 1) {
                    toPrint = (ForgeWorld) OTG.getWorld(argString[1]);
                    specifiedWorld = true;
                }
                ...
                if (toPrint == null) {
                    if (specifiedWorld) {
                        OTG.log(LogMarker.INFO, "Did not find a TC config for '{}'. Printing biome registry. Format is Id-RegistryKey"
                                                + ".", argString[1]);

                    } else {
                        OTG.log(LogMarker.INFO, "Printing biome registry. Format is Id-RegistryKey.");
                    }
                    ...
                    for (final Biome biome : registry) {
                        ...
                        if (generationId != savedId) {
                            if (!virtualHeader) {
                                virtualHeader = true;
                                OTG.log(LogMarker.INFO, "Printing Virtual biomes. Format is GenId-GenRegistryKey-SaveId-SaveRegistryKey"
                                                        + ".");
                            }
                            OTG.log(LogMarker.INFO, "  {}-{}-{}-{}", generationId, biome.getRegistryName(), savedId, registry.getValue
                                    (savedId).getRegistryName());
                        } else {
                            OTG.log(LogMarker.INFO, "  {}-{}", generationId, biome.getRegistryName());
                        }
                    }
                } else {
                    OTG.log(LogMarker.INFO, "Printing specific biomes for '{}'. Format is Id-RegistryKey.", toPrint.getName());
                    boolean virtualHeader = false;
                    ...
                    for (final LocalBiome tcBiome : toPrint.biomeNames.values().stream().sorted(Comparator.comparing(localBiome -> localBiome.getIds().getGenerationId())).collect(Collectors.toList())) {
                        ...
                        if (generationId != savedId) {
                            if (!virtualHeader) {
                                virtualHeader = true;
                                OTG.log(LogMarker.INFO, "Printing specific Virtual biomes for '{}'. Format is "
