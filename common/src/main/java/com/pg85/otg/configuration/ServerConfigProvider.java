...
public final class ServerConfigProvider implements ConfigProvider
{
    ...
    private void loadCustomObjects()
    {
        ...
        {
            OTG.log(LogMarker.WARN, "Failed to move old world"
        ...
        Map<String, CustomObjectLoader> objectLoaders = OTG.getCustomObjectManager().getObjectLoaders();

        customObjects = new CustomObjectCollection(objectLoaders, worldObjectsDir);
        customObjects.setFallback(OTG.getCustomObjectManager().getGlobalObjects());
        OTG.log(LogMarker.INFO, "{} world custom objects loaded.", customObjects.getAll().size());
    }
